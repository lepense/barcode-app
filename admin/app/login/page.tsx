...
export default function AdminLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // TODO: Implement admin login with Firebase Auth
    // 1. Sign in with email/password
    // 2. Verify admin custom claim or admin collection doc
    // 3. Set session cookie
    // 4. Redirect to dashboard
    setError("Firebase not yet configured. Set up Firebase to enable admin login.");
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-zinc-50 dark:bg-zinc-950">
      <div className="w-full max-w-sm bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-8">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button
            type="submit"
            className="w-full py-2 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 rounded-lg text-sm font-medium hover:bg-zinc-800 dark:hover:bg-zinc-100 transition-colors"
          >
            Sign In
          </button>
        </form>
